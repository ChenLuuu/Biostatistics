...
DF_INVCHI = 3  # degrees of freedom for inverse Chi-square distribution
SIGMA_B = 0.4
XI = 0.2
...
sigma_object_ts = SigmaObject(
    ...
    link="log",
    ran_dist="inverse-Chi",
    str_fixed=np.array([ALPHA0, ALPHA1]),
    fix_name="alpha",
    ran_name="a",
    disp_name="siga",
    str_disp=None  # No sigma_a for inverse Chi-square
)
...
for k in range(REP):
    ...
    while (nlme_fit is None or cd4_fit is None or TS is None or JM is None or 
        ...
        # Setting II: exp(a_i) ~ inverse Chi^2 distribution (df=3)
        # In R: a0 <- log(df_invChi/rchisq(n, df_invChi))
        # This is equivalent to log(df_invChi) - log(rchisq(n, df_invChi))
        chi2_samples = np.random.chisquare(DF_INVCHI, N)
        a0 = np.log(DF_INVCHI / chi2_samples)
...
with open(output_dir / "s2_results.pkl", 'wb') as f:
...
def create_visualization():
    ...
    Inverse Chi-square df: {DF_INVCHI}
